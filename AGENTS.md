...
## Версионирование конфигов (правило Hermes)

1. **Ветки = версии**: Каждая значимая версия конфигов Hermes сохраняется в отдельной ветке формата `hermes-config-YYYY-MM-DD` (например, `hermes-config-2026-06-12`).
2. **Откат**: Для восстановления предыдущей версии достаточно переключиться на нужную ветку.
3. **Текущая ветка**: `hermes-config` — всегда последняя актуальная версия.
4. **История**: Старые ветки не удалять — они нужны для отката.
5. **Создание новой версии**: После значимых изменений (новые скиллы, обновление памяти, смена конфигов) создавать новую версионную ветку.
